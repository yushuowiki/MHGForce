...
class HyperGCN(nn.Module):
    def __init__(self, num_features, num_classses, args):
        ...
        d, l, c = num_features, args.depth, num_classses
        ...
        self.layers = nn.ModuleList([HyperGraphConvolution(
            h[i], h[i+1], (not args.fast)) for i in range(l)])
        self.do, self.l = args.dropout, args.depth
        self.structure, self.m = None, args.mediators
        self.fast = args.fast
...
# class HyperGCN(nn.Module):
#     def __init__(self, V, E, X, args, device='cpu'):
#         """
#         d: initial node-feature dimension
#         h: number of hidden units
#         c: number of classes
#         """
#         super(HyperGCN, self).__init__()
#         d, l, c = args.d, args.depth, args.c # hidden, layers, class
...
#         self.layers = nn.ModuleList([HyperGraphConvolution(h[i], h[i+1], reapproximate, self.device) for i in range(l)])
#         self.do, self.l = args.dropout, args.depth
...
#     def __init__(self, a, b, reapproximate=True, device='cpu'):
#         super(HyperGraphConvolution, self).__init__()
#         self.a, self.b = a, b
...
#     edges, weights = [], {}
#     rv = np.random.rand(X.shape[1])
#     # rv = torch.rand(X.shape[1]).to(device)
#     for k in E.keys():
#         hyperedge = list(E[k])
        
#         p = np.dot(X[hyperedge], rv)   #projection onto a random vector rv
#         # p = torch.dot(X[hyperedge], rv)
#         s, i = np.argmax(p), np.argmin(p)
#         # s, i = torch.argmax(p), torch.argmin(p)
#         Se, Ie = hyperedge[s], hyperedge[i]
