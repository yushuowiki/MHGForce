...
class HNHNConv(MessagePassing):
    ...
    def forward(self, x, data):
        ...
        out = self.propagate(hyperedge_index[[1,0]], x=out, norm=data.D_v_alpha_inv,
...
class HNHN(nn.Module):
    ...
    def __init__(self, num_features, num_classes, args):
        ...
        if self.num_layers == 1:
            self.convs.append(HNHNConv(num_features, args.nhid, num_classes,
                                       nonlinear_inbetween=args.HNHN_nonlinear_inbetween))
        else:
            self.convs.append(HNHNConv(num_features, args.nhid, args.nhid,
                                       nonlinear_inbetween=args.HNHN_nonlinear_inbetween))
            for _ in range(self.num_layers - 2):
                self.convs.append(HNHNConv(args.nhid, args.nhid, args.nhid,
                                           nonlinear_inbetween=args.HNHN_nonlinear_inbetween))
            self.convs.append(HNHNConv(args.nhid, args.nhid, num_classes,
                                       nonlinear_inbetween=args.HNHN_nonlinear_inbetween))
